...
const RoomCard = ({ room }) => {
  const { mapFacilityIdsToNames, loading: facilitiesLoading } = useFacilities();
  ...
  const facilities = Array.isArray(rawFacilities) && rawFacilities.length > 0 
    ...
    : ['AC', 'Fan', 'Wifi', 'Attached Bathroom']; // Fallback for rooms without facilities
  
  const genderPreference = room.genderPreference || room.gender_preference || 'Mixed';
  
  // Debug logging to see facility mapping in action
  console.log('🏢 RoomCard facility mapping:', {
    roomNumber,
    rawFacilities,
    mappedFacilities: facilities,
    facilitiesLoading
  });
