...
const RoomManagement = () => {
  const { rooms, addRoom, fetchRooms, isLoading, isAuthenticated, userRole, authToken } = useAppContext();
  ...
  const handleRefreshRooms = async () => {
    try {
      ...
      console.log('🔑 Current auth token:', authToken ? `${authToken.substring(0, 10)}...` : 'null');
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      if (isEditMode && editingRoom) {
        ...
        console.log('Updating room:', editingRoom.id, roomDataToSubmit);
        alert('Room update functionality will be implemented with API integration.');
      } else {
      ...
      alert('Failed to create/update room. Please try again.');
    }
  };
  ...
  const filteredRooms = useMemo(() => {
    ...
    return rooms.filter(room => {
      ...
      // Floor restrictions based on gender
      let floorMatch = floorFilter === 'All' || roomFloor.toString() === floorFilter;

      // Apply gender-based floor restrictions
      if (genderFilter === 'Male') {
        // Males can only access: Ground, First, Second floors
        const maleAllowedFloors = ['ground', 'first', 'second', '0', '1', '2'];
        const roomFloorLower = roomFloor.toString().toLowerCase();
        const isFloorAllowedForMale = maleAllowedFloors.some(floor => roomFloorLower.includes(floor));
        floorMatch = floorMatch && (floorFilter === 'All' ? isFloorAllowedForMale : floorMatch);
      } else if (genderFilter === 'Female') {
        // Females can access: Ground, First, Second, Third, Fourth floors
        const femaleAllowedFloors = ['ground', 'first', 'second', 'third', 'fourth', '0', '1', '2', '3', '4'];
        const roomFloorLower = roomFloor.toString().toLowerCase();
        const isFloorAllowedForFemale = femaleAllowedFloors.some(floor => roomFloorLower.includes(floor));
        floorMatch = floorMatch && (floorFilter === 'All' ? isFloorAllowedForFemale : floorMatch);
      }

      return searchMatch && typeMatch && genderMatch && floorMatch;
    });
      ...
      <Card>
        ...
        <Table headers={['Room No', 'Type', 'Floor', 'Occupancy', 'Price/Year', 'Gender', 'Status', 'Actions']}>
          {filteredRooms.map((room, index) => (
            <TableRow key={room.id || room._id || index}>
              ...
              <TableCell>
                {room.occupiedCots !== undefined && room.totalCots !== undefined
                  ? `${room.occupiedCots}/${room.totalCots} cots`
                  : room.occupied_cots !== undefined && room.total_cots !== undefined
                    ? `${room.occupied_cots}/${room.total_cots} cots`
                    : 'N/A'
                }
              </TableCell>
              <TableCell>
        ...
        {filteredRooms.length === 0 && (
          <div className="text-center p-8">
            {!isAuthenticated ? (
              <p className="text-red-600 font-medium">⚠️ Not authenticated. Please log in to view rooms.</p>
            ) : isLoading ? (
